...
from neon.layers import Conv, Pooling, GeneralizedCost, Affine, DeepBiRNN, RecurrentLast, Dropout
from neon.optimizers import Adadelta, GradientDescentMomentum
from neon.transforms import Rectlin, Softmax, CrossEntropyBinary, Misclassification, Logistic
...
def run(train, test):
    ...
    layers = [
        # Conv((3, 3, 15), init=init, activation=Rectlin(),
        #      strides=dict(str_h=1, str_w=2)),
        Conv((3, 3, 15), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 30), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 30), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 60), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 60), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 60), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 30), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 30), init=init, batch_norm=True, activation=Rectlin()),
        Conv((3, 3, 15), init=init, batch_norm=True, activation=Rectlin()),
        Affine(100, init=init, batch_norm=True, activation=Rectlin()),
        Affine(nout=common['nclasses'], init=init, activation=Softmax())]
...
common_params = dict(sampling_freq=2000, clip_duration=1700, frame_duration=64, overlap_percent=50)
